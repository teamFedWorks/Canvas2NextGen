...
class CanonicalPipeline:
    ...
    def _enrich(self, canonical: CanonicalCourse) -> CanonicalCourse:
        ...
        # Build asset lookup for link validation
        asset_lookup = {asset.identifier: asset for asset in canonical.assets}
        enricher = ContentEnricher(asset_lookup=asset_lookup)
        
        # Enrich course in-place
        result = enricher.enrich_course(canonical)
        
        if result.warnings:
            canonical.parsing_warnings.extend(result.warnings)
